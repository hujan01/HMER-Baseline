...
Date: 2020-12-29 19:29:44
LastEditTime: 2020-12-29 19:45:11
'''
import os
import sys
...
latex_root_path = 'data/' # root path
gtd_root_path = 'data/CROHME_gtd' # gtd file output path
latex_files = ['test2016.txt']  # label file path

for latexF in latex_files:
    latex_file = latex_root_path + latexF
    gtd_path = gtd_root_path + '/' + latexF[:-4] + '/'
    if not os.path.exists(gtd_path):
        os.makedirs(gtd_path)
    ...
    with open(latex_file) as f:
        ...
        for process_num, line in enumerate(lines):
            ...
            if len(parts) < 2:
                ...
                continue
            key = parts[0]
            f_out = open(gtd_path + key + '.gtd', 'w') # 写入.gtd
            raw_cap = parts[1:]
            cap = []  
            for w in raw_cap:
                if w not in ['\limits']:
